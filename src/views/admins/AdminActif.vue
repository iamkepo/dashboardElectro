          ...
          <div class="card rounded shadow">
            <div class="card-body">
              <div class="d-flex justify-content-between mb-4">
                <h3 class="text-left mb-3">Liste des administrateurs actifs</h3>
                <div>
                ...
                <div v-else>
                  <div class="d-flex mb-4 px-3 justify-content-between align-items-center">
                    <div>Affichage de 
                      ...
                      sur <strong>{{total}}</strong> Admins actifs
                    </div>
                    <div>
                        ...
                        <tr v-for="(admin, i) in admins" :key="i">
                          ...
                          <td class="text-right">
                            ...
                            <btn-dropdown btnClass="btn btn-secondary btn-sm" icon="ellipsis-h"
                              :items="[ 
                                { text: 'Modifier les infos', onClick: () => {editAdmin(admin._id)}},
                                { text: 'Désactiver l\'admin', onClick: () => {desactivateAdmin(admin._id)}}
                              ]"/>
                          </td>
                        </tr>
                      </tbody>
...
export default {
  ...
  methods: {
    ...
    editAdmin (id) {
      this.$router.push({name: 'admins-edit', query: {adminId: id}});
    },
    showAdmin (id) {
    ...
    desactivateAdmin (id) {
      this.$confirmAction(
        ...
        'Vous voulez désactiver ce admin', 
        'Oui, nommer', 
        '#045480'
        ).then((result) => {
          if (result.value) {
            this.$service.update('admins', id, {status: 'inactive'}, (err, data) => {
              if (err) {
